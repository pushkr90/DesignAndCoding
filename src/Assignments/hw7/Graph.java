...
class Graph
{
  ...
  public int insertOrFind(String name, boolean mustbethere)
  {
    ...
    if (mustbethere)
    {
  	...
  	//System.out.println(n.name+"PUSHKAR"+numV);
  	nodes.add(n) ;
  ...
  public void createEdge(int from, int to, double w, boolean fanout)
  {
	  ...
	  //System.out.println(n.name+"RAJ");
  	u.myassert(n.name == from) ;
  	edge e = new edge(to,w) ;
  	//System.out.println(e.other+"RAJ");
  	if (fanout == true)
  	{
  		numE++ ;
  		n.fanouts.add(e) ;
  ...
  public void buildSnakeAndLadderGraph(int n, int[][] l, int [][] s) {
	...
	public void dfs(String t, boolean[] cycle, int [] work, int [] size, int [] dfsorder) {
		GraphDfs b = new GraphDfs(t,this,cycle,work,size,dfsorder);
	}
